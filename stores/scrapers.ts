...
const executeScraper = async (
  ...
  // read file version
  let version: string = await invoke("plugin:file-version|get_file_version", {
    path: scraper.path,
  });
...
const registerFileWatcher = async (
  ...
  try {
    ...
    message(`Could not activate file watcher: \n${error}`, {
      title: "Tauri | Enable file scraper",
      type: "warning",
    });
...
export const useScraperStore = defineStore("scraper-store", {
  ...
  actions: {
    async init() {
      ...
      const broker: MqttBroker = await tauriStore.get(
        "settings-file-mqtt-broker"
      );
      this.mqttBrokerSettings =
        broker ||
        ({
          clientId: "tauri-mqtt-client",
          host: "localhost",
      ...
      for (const scraper of this.fileScrapers) {
        if (scraper.enabled)
          await registerFileWatcher(
            scraper,
            this.mqttBrokerSettings,
            this.mqttBrokerState
          );
      }
    ...
    removeFileScraper(id: string) {
      this.fileScrapers.splice(
        this.fileScrapers.findIndex((obj) => obj.id === id),
        1
      );
    },
    async toggleEnableState(id: string) {
      ...
      // handle file watcher
      let scraper: Scraper = this.fileScrapers.filter(
      ...
      if (scraper.enabled) {
        ...
        await executeScraper(
          scraper,
          this.mqttBrokerSettings,
          this.mqttBrokerState.connected
        );
      } else {
        await unregisterFileWatcher(scraper);
      }
    ...
    toggleBrokerSecurity() {
      this.mqttBrokerSettings.protocol =
        this.mqttBrokerSettings.protocol === MqttProtocol.mqtt
          ? MqttProtocol.mqtts
          : MqttProtocol.mqtt;
    },
  },
  getters: {
    ...
    enabledFileScrapers: (state) =>
      state.fileScrapers.filter((scraper) => scraper.enabled),
  },
  debounce: {
    tauriSave: 1000,
  },
